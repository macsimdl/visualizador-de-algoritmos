	...
	if(contador==0){
		if(iniciado){
				if(explorados.length > 0){
					var actual = explorados[0];
					console.log(values[actual].num);
					extremo = false;
					if(values[actual].iz != -1 || values[actual].der != -1){
						if(flag == 0){
							if(values[actual].iz != -1){
							...
							if(explorados[0] == 0){
								texto = texto + values[explorados[0]].num;
							}
							else{
								texto = texto + ", " + values[explorados[0]].num;
							}
...
function drawcaminos(){
	if(caminos.length > 0){
		...
		for(var i = 0; i<caminos.length-2; ++i){
			console.log(values[caminos[i]].num);
			console.log(values[caminos[i+1]].num);
		...
		console.log(values[caminos[caminos.length-1]].num);
		console.log(values[caminos[caminos.length-2]].num);
